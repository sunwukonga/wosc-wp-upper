...
class PortfolioList implements Lib\ShortcodeInterface {
				    ...
				    'params'                    => array(
					    ...
					    array(
						    ...
						    'value'       => array(
								...
								esc_html__( 'Gallery - Overlay With Info', 'eltdf-core' )       => 'gallery-overlay-with-info',
    ...
    */
	...
	/**
	 * Returns hover background color for items
	 *
	 * @return string
	 */
	public function getItemHoverBackgroundColor($type = ''){
		$portfolio_background_color_style = '';
		$portfolio_background_color = get_post_meta(get_the_ID(), 'eltdf_portfolio_background_color_meta', true);
		
		if ($portfolio_background_color == ''){
			return;
		}

		if ($type == 'transparent'){
			$portfolio_background_color = mrseo_elated_rgba_color($portfolio_background_color,0.9);
		}

		$portfolio_background_color_style = 'background-color:'.$portfolio_background_color;
		
		return $portfolio_background_color_style;
	}
