(function($) {
	...
	function eltdfGenerateSerializedString() {
		...
		if($('input[name="eltdf_menu_options"]').length) {
			$('input[name="eltdf_menu_options"]').val(encodeURIComponent(dataArrayString));
		} else {
    ...
    function eltdfUpdateIconOptions() {
        ...
        navForm.on('change', '[data-icon-pack]', function() {
            ...
            var iconDropdown   = $(this).parents('p').first().next('.eltdf-icon-select-holder').find('select');
