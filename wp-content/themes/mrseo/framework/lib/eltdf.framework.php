...
class MrSeoElatedFramework
{
    ...
    private function __construct() {
        $this->eltdfOptions = MrSeoElatedOptions::get_instance();
        $this->eltdfMetaBoxes = MrSeoElatedMetaBoxes::get_instance();
    }
    ...
    public function setSkin(MrSeoElatedSkinAbstract $skinObject) {
        $this->skin = $skinObject;
    }
}

/**
 * Class MrSeoElatedSkinManager
 *
...
/**
 * Class MrSeoElatedSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class MrSeoElatedSkinAbstract
{
    /**
     * @var string
     */
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if ($template !== '') {
            include(mrseo_elated_find_template_path('framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
        <script>
            window.eltdfSCIcon = '<?php echo mrseo_elated_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
...
class MrSeoElatedOptions
{
    ...
    public function getOptionValue($key) {
        ...
        if (array_key_exists($key, $mrseo_elated_options)) {
            return $mrseo_elated_options[$key];
...
/*
   Class: MrSeoElatedAdminPage
   A class that initializes Elated Admin Page
*/

class MrSeoElatedAdminPage implements iMrSeoElatedLayoutNode
{
    ...
    function __construct($slug = "", $title = "", $icon = "") {
        $this->layout = array();
        $this->factory = new MrSeoElatedFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iMrSeoElatedRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: MrSeoElatedMetaBoxes
...
/*
   Class: MrSeoElatedMetaBox
   A class that initializes Elated Meta Box
*/

class MrSeoElatedMetaBox implements iMrSeoElatedLayoutNode
{
    ...
    public function setTitle($label) {
        ...
        return $mrseo_elated_Framework->getSkin()->getSkinLabel() . ' ' . $label;
    }
}

if (!function_exists('mrseo_elated_init_framework_variable')) {
    function mrseo_elated_init_framework_variable() {
        global $mrseo_elated_Framework;

        $mrseo_elated_Framework = MrSeoElatedFramework::get_instance();
        $eltdfSkinManager = new MrSeoElatedSkinManager();
        $mrseo_elated_Framework->setSkin($eltdfSkinManager->getSkin());
    }

    add_action('mrseo_elated_before_options_map', 'mrseo_elated_init_framework_variable');
}
?>