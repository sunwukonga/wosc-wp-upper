...
include_once 'eltdf-twitter-helper.php';
...
class ElatedTwitterApi {
    ...
    const REQUEST_TOKEN_FIELD = 'eltdf_twitter_request_token';
    const REQUEST_TOKEN_SECRET_FIELD = 'eltdf_twitter_request_token_secret';
    const ACCESS_TOKEN_FIELD = 'eltdf_twitter_access_token';
    const ACCESS_TOKEN_SECRET_FIELD = 'eltdf_twitter_access_token_secret';
    const AUTHORIZE_TOKEN_FIELD = 'eltdf_twitter_authorize_token';
    const AUTHORIZE_VERIFIER_FIELD = 'eltdf_twitter_authorize_verifier';
    const USER_ID_FIELD = 'eltdf_twitter_user_id';
    const USER_SCREEN_NAME_FIELD = 'eltdf_twitter_screen_name';
    ...
    public function obtainRequestToken() {
        ...
        if(is_wp_error($response)) {
            ...
            $responseObj->message = esc_html__('Internal WP error', 'eltdf');
        } else {
            ...
            if(!empty($responseBody)) {
                ...
                if((is_array($responseParsed) && count($responseParsed)) &&
                    ...
                    if(!empty($responseObj->redirectUrl)) {
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Redirect URL couldn\t not be generated', 'eltdf');
                    } else {
                    ...
                    $responseObj->status = false;
                    $responseObj->message = esc_html__('Couldn\'t connect with Twitter API', 'eltdf');
                }
            }
        }
        ...
        if(!empty($authorizeVerifier) && !empty($authorizeToken)) {
            ...
            if(is_wp_error($response)) {
                ...
                if(is_array($responseParsed) && count($responseParsed)
                    ...
                    $responseObj->status = true;
                    $responseObj->message = esc_html__('Access token obtained', 'eltdf');
                }
            }
        } else {
            $responseObj->status = false;
            $responseObj->message = esc_html__('Authorize token and it\'s secret were not obtainer', 'eltdf');
        }
    ...
    public function fetchTweets($userId = '', $count = '', $transient = array()) {
        ...
        if(!$this->transientExists($transient)) {
            if($userId && $accessToken) {
                ...
                if(is_wp_error($response)) {
                    ...
                    if(isset($response['response']['code']) && $response['response']['code'] == 200) {
                        ...
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Couldn\'t connect with Twitter', 'eltdf');
                    }
                }
            } else {
                $responseObj->status = false;
                $responseObj->message = esc_html__('It seams like you haven\t connected with your Twitter account', 'eltdf');
            }
        } else {
            ...
            if(!empty($transientContent)) {
                ...
                $responseObj->status = false;
                $responseObj->message = esc_html__('Couldn\'t retreive content from database', 'eltdf');
            }
        }
